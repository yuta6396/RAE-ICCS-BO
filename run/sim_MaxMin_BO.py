...
base_dir = f"../test_result/MaxMin/t=0-{Objective_T_period}_{Control_X_size}-{Control_Y_size}-{Control_Z_size}grids_FET={opt_num}_trials={trial_base}-{trial_base+trial_num-1}_{current_time}"
...
def predict(inputs, f):
    ...
    # 累積降水量の値の計算
    result = np.zeros((batch_size, Objective_X_high-Objective_X_low, Objective_Y_high-Objective_Y_low)) 
    for loop in range(0, Objective_T_period):
        ...
        result += calc_objective_func(loop)

    # 累積降水量から最大値を抽出
    max_indices = [np.unravel_index(np.argmax(result[i]), result[i].shape) for i in range(batch_size)]
    max_values = np.max(result, axis=(1, 2))# 各 i に対する最大値
    max_values_list = max_values.tolist()
    print(f"{max_values_list=}")
    for i, idx in enumerate(max_indices):
        print(f"Max value index for result[{i}]: {idx}, Value: {result[i][idx]} [mm]")
        f.write(f"Max value index for result[{i}]: {idx}, Value: {result[i][idx]} [mm]\n")
    return max_values_list

def calc_objective_func(loop):
    result = np.zeros((batch_size, Objective_X_high-Objective_X_low, Objective_Y_high-Objective_Y_low)) # ステップごとの結果を初期化
    for i in range(batch_size):
        for pe in range(nofpe):
            sub_history_file = f"000{i}/history_d01.pe######.nc"
        ...
        for j in range(Objective_X_low,Objective_X_high):
            for k in range(Objective_Y_low,Objective_Y_high):
                # for l in range(1, Objective_T_period+1):
                    result[i, j-Objective_X_low, k-Objective_Y_low] = dat[1, 0, k, j]*TIME_INTERVAL
    return result
...
def init_val_intervation(num,LB_Control_X,LB_Control_Y, Control_MOMX, Control_MOMY, f):
    ...
    global org_file
    f.write(f"(X, Y) = ({LB_Control_X}, {LB_Control_Y}),  (d_x, d_y) = ({Control_MOMX}, {Control_MOMY}\n")
...
def sim(trial_i, LB_Control_X,LB_Control_Y,Control_MOMX, Control_MOMY):
    ...
    for loop in range(0, Objective_T_period):
        TEST_prec_matrix, CTRL_prec_matrix, TEST_CTRL_prec_matrix = state_update(trial_i, loop, TEST_prec_matrix, CTRL_prec_matrix, TEST_CTRL_prec_matrix)
    
    TEST_sum_result = np.sum(TEST_prec_matrix, axis=0) 
    CTRL_sum_result = np.sum(CTRL_prec_matrix, axis=0) 
    # 最大値を取得
    TEST_max_value = np.max(TEST_sum_result)
    CTRL_max_value = np.max(CTRL_sum_result)
    print(f"{TEST_max_value=}\n{CTRL_max_value=}")
    # 最大値のインデックスを取得
    TEST_max_index = np.unravel_index(np.argmax(TEST_sum_result), TEST_sum_result.shape)
    CTRL_max_index = np.unravel_index(np.argmax(CTRL_sum_result), CTRL_sum_result.shape)

    return TEST_max_value, TEST_max_index, CTRL_max_value, CTRL_max_index, TEST_prec_matrix, CTRL_prec_matrix ,TEST_CTRL_prec_matrix
     
def state_update(trial_i, loop, TEST_prec_matrix, CTRL_prec_matrix, TEST_CTRL_prec_matrix):
    ...
    return TEST_prec_matrix, CTRL_prec_matrix ,TEST_CTRL_prec_matrix


def plot_PREC(trial_i, TEST_prec_matrix, TEST_CTRL_prec_matrix, Objective_ratio):
    """
    ...
    im = ax.pcolormesh(slon, slat, prec_sum_matrix, cmap=cmap, norm=norm,shading='auto', transform=ccrs.PlateCarree())

    # カラーバーを追加
    cbar = fig.colorbar(im, ax=ax, orientation='vertical', pad=0.05, aspect=30)
    cbar.set_label('mm', fontsize=18)
    cbar.ax.tick_params(labelsize=15)
    filename = os.path.join(base_dir, "PREC-heatmap", f"LonLat_seed={trial_i}_{Objective_ratio}%.pdf")
...
def main():
    ...
    with open(log_file, 'w') as f, open(summary_file, 'w') as f_s, open(analysis_file, 'w') as f_a:
        ...
        for trial_i in range(trial_base, trial_base+trial_num):
            ...
            TEST_max_value, TEST_max_index, CTRL_max_value, CTRL_max_index, TEST_prec_matrix, CTRL_prec_matrix ,TEST_CTRL_prec_matrix=sim(trial_i, optimal_inputs[0],optimal_inputs[1],optimal_inputs[2], optimal_inputs[3])#  change
            ...
            print(f"{TEST_max_value=} ({TEST_max_index=})")
            print(f"{CTRL_max_value=} ({CTRL_max_index=})")
            f_s.write(f"{TEST_max_value=} ({TEST_max_index=})")
            f_s.write(f"{CTRL_max_value=} ({CTRL_max_index=})")
            f_s.write(f"%={TEST_max_value/CTRL_max_value*100}%\n\n")
            Objective_ratio = (CTRL_max_value-TEST_max_value)*100/CTRL_max_value
            f_s.write(f"Reduction={Objective_ratio}%\n\n")
            plot_PREC(trial_i, TEST_prec_matrix,  TEST_CTRL_prec_matrix, Objective_ratio)
