...
# optimal_inputs= [50,50,50,50,50,50,50,50,50,50,50,50]
...
base_dir = f"../test_result/reduce_MOMXY_{Objective_T_period}inputs/t=0-{Objective_T_period}_CVar={Control_Var}_{Control_X_size}-{Control_Y_size}-{Control_Z_size}grids_FET={opt_num}_trials={trial_base}-{trial_base+trial_num-1}_{current_time}"
...
def intervation(loop, inputs, f):
    ...
    if loop < Objective_T_period:
        for i in range(batch_size):
            ...
            init_val_intervation(i,inputs[i][loop*2],inputs[i][loop*2+1], f) # ここで値を書き換える
    return
        ...
        for pe in range(nofpe):
            ...
            with netCDF4.Dataset(init) as src, netCDF4.Dataset(output, "w") as dst:
                ...
                for name, variable in src.variables.items():
                    ...
                    if name == "MOMX":
                        var = src[name][:]
                        dst[name][:] = control_var_operation(var, pe, optimal_inputs[loop*2],optimal_inputs[loop*2+1], Control_Y_size, Control_X_size, 0)
                    elif name == "MOMY":
                        var = src[name][:]
                        dst[name][:] = control_var_operation(var, pe, optimal_inputs[loop*2],optimal_inputs[loop*2+1], Control_Y_size, Control_X_size, 0)
                    else:
...
def main():
    ...
    # 入力次元と最小値・最大値の定義
    bounds = [Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size),Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size),Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size),Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size),Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size), Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size)] # Int(2, 4) なら2, 3, 4からランダム
    ...
    with open(log_file, 'w') as f, open(summary_file, 'w') as f_s, open(analysis_file, 'w') as f_a:
        ...
        for trial_i in range(trial_base, trial_base+trial_num):
            ...
            for i in range(2):  # 最初の10点はRS
                ...
                random_samples3 = [[random.randint(Control_X_low, Control_X_high-Control_X_size),] for _ in range(batch_size)]
                random_samples4 = [[random.randint(Control_Y_low, Control_Y_high-Control_Y_size) ] for _ in range(batch_size)]
                ...
                random_samples7 = [[random.randint(Control_X_low, Control_X_high-Control_X_size),] for _ in range(batch_size)]
                random_samples8 = [[random.randint(Control_Y_low, Control_Y_high-Control_Y_size) ] for _ in range(batch_size)]
                ...
                random_samples11 = [[random.randint(Control_X_low, Control_X_high-Control_X_size),] for _ in range(batch_size)]
                random_samples12 = [[random.randint(Control_Y_low, Control_Y_high-Control_Y_size) ] for _ in range(batch_size)]
                # リストを結合してサンプルを作成
                combined_samples = [sample1 + sample2+ sample3 + sample4 + sample5 + sample6+ sample7 + sample8 + sample9 + sample10+ sample11 + sample12 for sample1 , sample2, sample3 , sample4 , sample5 , sample6, sample7 , sample8 , sample9 , sample10, sample11 , sample12 in zip(random_samples1 , random_samples2, random_samples3 , random_samples4 , random_samples5 , random_samples6, random_samples7 , random_samples8 , random_samples9 , random_samples10, random_samples11 , random_samples12)]
