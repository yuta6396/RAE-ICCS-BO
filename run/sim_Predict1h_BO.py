...
base_dir = f"../test_result/1h_predict/t=0-{Objective_T_period}_{Control_X_size}-{Control_Y_size}-{Control_Z_size}grids_FET={opt_num}_trials={trial_base}-{trial_base+trial_num-1}_{current_time}"
...
def predict(inputs, f):
    ...
    result_step = calc_objective_func(f)
    for i in range(0, batch_size):
        result[i] += result_step[i]
    # print(result)
    print(f"t=0-1:Sum of PREC in X[{Objective_X_low}, {Objective_X_high-1}], Y[{Objective_Y_low}, {Objective_Y_high-1}] = {result} [mm/h]\n")
    f.write(f"t=0-1:Sum of PREC in X[{Objective_X_low}, {Objective_X_high-1}], Y[{Objective_Y_low}, {Objective_Y_high-1}] = {result} [mm/h]\n")
    return result
...
def main():
    ...
    result_file = os.path.join(base_dir, "summary", f"BO_result.txt")
    ...
    with open(log_file, 'w') as f, open(summary_file, 'w') as f_s, open(analysis_file, 'w') as f_a, open(result_file, 'w') as f_r:
        BO_data = []
        BO_search_data = []
        for trial_i in range(trial_base, trial_base+trial_num):
            ...
            f_r.write(f'{trial_i=}')
            f_r.write(f'{opt_num}回の評価で得られた最適解：{optimal_inputs}\n')
            f_r.write(f"{100 - TEST_prec_sum/CTRL_prec_sum*100}\n\n")
