...
# 制御対象範囲
Control_X_low = 45
Control_X_high =85#AX =90
Control_Y_low = 5
Control_Y_high = 85 #AX =90
...
#　介入幅
Control_MOMXY_low = -20   #30~30でエラー確認済み(MOMY)
Control_MOMXY_high = 20
Control_QR_low =0
Control_QR_high = 0.05
Control_RHOT_low = 1   #30~30でエラー確認済み(MOMY)
Control_RHOT_high = 1.05

"""
最適解の候補
Input_MOMXY = [71, 57, 20,-20] #和
Input_RHOT = [70, 40 ,1] #積
Input_QR =    [71, 40, 0.05] #和
"""
...
base_dir = f"../test_result_VariousInputs/t=0-{Objective_T_period}_{Control_X_size}-{Control_Y_size}-{Control_Z_size}grids_FET={opt_num}_trials={trial_base}-{trial_base+trial_num-1}_{current_time}"
...
def predict(inputs, f):
    ...
    for i in range(batch_size):
        ...
        init_val_intervation(i,inputs[i], f) # ここで初期値を書き換える
...
def init_val_intervation(num, inputs, f):
    """
    ...
    for pe in range(nofpe):
        ...
        with netCDF4.Dataset(sub_init) as src, netCDF4.Dataset(output, "w") as dst:
            ...
            for name, variable in src.variables.items():
                ...
                var = src[name][:]
                if name == "MOMX":
                    dst[name][:] = control_var_add_operation(var, pe, inputs[0], inputs[1],  0, inputs[2])
                elif name == "MOMY":
                    dst[name][:] = control_var_add_operation(var, pe, inputs[0], inputs[1],  0, inputs[3])
                elif name == "QR":
                    dst[name][:] = control_var_add_operation(var, pe, inputs[4], inputs[5],  0, inputs[6])
                elif name == "RHOT":
                    dst[name][:] = control_var_product_operation(var, pe, inputs[7], inputs[8], 0, inputs[9])
                else:
                    dst[name][:] = src[name][:]
...
def main():
    ...
    with open(config_file_path,  "w") as f:
        ...
        # 介入幅
        f.write("\n# 介入幅\n")
        f.write(f"{Control_MOMXY_low=}\n")
        f.write(f"{Control_MOMXY_high=}\n")
        f.write(f"{Control_QR_low=}\n")
        f.write(f"{Control_QR_high=}\n")        
        f.write(f"{Control_RHOT_low=}\n")
        f.write(f"{Control_RHOT_high=}\n")  
        # BOの獲得関数
        f.write("\n# BOの獲得関数\n")
    ...
    # 入力次元と最小値・最大値の定義
    bounds = [ Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size) ,Real(Control_MOMXY_low,Control_MOMXY_high), Real(Control_MOMXY_low,Control_MOMXY_high),
               Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size) ,Real(Control_QR_low,Control_QR_high), 
               Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size) ,Real(Control_RHOT_low,Control_RHOT_high)] # Int(2, 4) なら2, 3, 4からランダム
    ...
    with open(log_file, 'w') as f, open(summary_file, 'w') as f_s, open(analysis_file, 'w') as f_a:
        ...
        for trial_i in range(trial_base, trial_base+trial_num):
            ...
            for i in range(2):  # 最初の10点はRS
                ...
                random_samples3 = [[random.uniform(Control_MOMXY_low,Control_MOMXY_high)] for _ in range(batch_size)]
                random_samples4 = [[random.uniform(Control_MOMXY_low,Control_MOMXY_high)] for _ in range(batch_size)]
                ...
                random_samples7 = [[random.uniform(Control_QR_low,Control_QR_high)] for _ in range(batch_size)]
                random_samples8 = [[random.randint(Control_X_low, Control_X_high-Control_X_size),] for _ in range(batch_size)]
                random_samples9 = [[random.randint(Control_Y_low, Control_Y_high-Control_Y_size) ] for _ in range(batch_size)]
                random_samples10 = [[random.uniform(Control_RHOT_low,Control_RHOT_high)] for _ in range(batch_size)]
                # リストを結合してサンプルを作成
                combined_samples = [sample1 + sample2+ sample3 + sample4 + sample5 + sample6+ sample7 + sample8 + sample9 + sample10
                for sample1 , sample2, sample3 , sample4 , sample5 , sample6, sample7 , sample8 , sample9 , sample10
                in zip(random_samples1 , random_samples2, random_samples3 , random_samples4 , random_samples5 , random_samples6, random_samples7 , random_samples8 , random_samples9 , random_samples10)]
            ...
            TEST_prec_sum, CTRL_prec_sum, TEST_prec_matrix, CTRL_prec_matrix , TEST_CTRL_prec_matrix=sim(trial_i, optimal_inputs)#  change
