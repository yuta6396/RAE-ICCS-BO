...
base_dir = f"../test_result/reduce_MOMXY_{Objective_T_period}inputs_4var/{Control_X_size}-{Control_Y_size}-{Control_Z_size}grids_FET={opt_num}_trials={trial_base}-{trial_base+trial_num-1}_{current_time}"


def intervation(loop, inputs, f):
    # 介入は1h前までOK
    if loop < Objective_T_period:
        for i in range(batch_size):
            ...
            init_val_intervation(i,inputs[i][loop*4], inputs[i][loop*4+1], inputs[i][loop*4+2], inputs[i][loop*4+3], f) # ここで値を書き換える
    return
    ...
    for pe in range(nofpe):
        ...
        with netCDF4.Dataset(sub_init) as src, netCDF4.Dataset(output, "w") as dst:
            ...
            for name, variable in src.variables.items():
                ...
                if name == "MOMX":
                    var = src[name][:]
                    dst[name][:] = control_var_operation(var, pe, LB_Control_X, LB_Control_Y,  0, Control_MOMX)
                elif name == "MOMY":
                    var = src[name][:]
                    dst[name][:] = control_var_operation(var, pe, LB_Control_X, LB_Control_Y, 0, Control_MOMY)
                else:
                    dst[name][:] = src[name][:]
        subprocess.run(["cp", output, sub_init ])
    return

def control_var_operation(var, pe, LB_Control_X, LB_Control_Y, B_Control_Z, Control_Var):
...
def sim(trial_i, optimal_inputs, f):
    """
        ...
        for pe in range(nofpe):
            ...
            with netCDF4.Dataset(init) as src, netCDF4.Dataset(output, "w") as dst:
                ...
                for name, variable in src.variables.items():
                    ...
                    if name == "MOMX":
                        var = src[name][:]
                        dst[name][:] = control_var_operation(var, pe, optimal_inputs[loop*4], optimal_inputs[loop*4+1], 0, optimal_inputs[loop*4+2])
                    elif name == "MOMY":
                        var = src[name][:]
                        dst[name][:] = control_var_operation(var, pe, optimal_inputs[loop*4], optimal_inputs[loop*4+1], 0, optimal_inputs[loop*4+3])
                    else:
                        dst[name][:] = src[name][:]
...
def main():
    ...
    # 入力次元と最小値・最大値の定義
    bounds = [Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size),Real(Control_Var_low,Control_Var_high), Real(Control_Var_low,Control_Var_high), Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size),Real(Control_Var_low,Control_Var_high), Real(Control_Var_low,Control_Var_high)
            ,Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size),Real(Control_Var_low,Control_Var_high), Real(Control_Var_low,Control_Var_high),Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size),Real(Control_Var_low,Control_Var_high), Real(Control_Var_low,Control_Var_high)
            ,Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size),Real(Control_Var_low,Control_Var_high), Real(Control_Var_low,Control_Var_high),Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size),Real(Control_Var_low,Control_Var_high), Real(Control_Var_low,Control_Var_high)] # Int(2, 4) なら2, 3, 4からランダム
    ...
    with open(log_file, 'w') as f, open(summary_file, 'w') as f_s, open(analysis_file, 'w') as f_a:
        ...
        for trial_i in range(trial_base, trial_base+trial_num):
            ...
            for i in range(2):  # 最初の10点はRS
                ...
                random_samples5 = [[random.randint(Control_X_low, Control_X_high-Control_X_size),] for _ in range(batch_size)]
                random_samples6 = [[random.randint(Control_Y_low, Control_Y_high-Control_Y_size) ] for _ in range(batch_size)]
                random_samples7 = [[random.uniform(Control_Var_low,Control_Var_high)] for _ in range(batch_size)]
                random_samples8 = [[random.uniform(Control_Var_low,Control_Var_high)] for _ in range(batch_size)]
                random_samples9 = [[random.randint(Control_X_low, Control_X_high-Control_X_size),] for _ in range(batch_size)]
                random_samples10 = [[random.randint(Control_Y_low, Control_Y_high-Control_Y_size) ] for _ in range(batch_size)]
                random_samples11 = [[random.uniform(Control_Var_low,Control_Var_high)] for _ in range(batch_size)]
                random_samples12 = [[random.uniform(Control_Var_low,Control_Var_high)] for _ in range(batch_size)]
                random_samples13 = [[random.randint(Control_X_low, Control_X_high-Control_X_size),] for _ in range(batch_size)]
                random_samples14 = [[random.randint(Control_Y_low, Control_Y_high-Control_Y_size) ] for _ in range(batch_size)]
                random_samples15 = [[random.uniform(Control_Var_low,Control_Var_high)] for _ in range(batch_size)]
                random_samples16 = [[random.uniform(Control_Var_low,Control_Var_high)] for _ in range(batch_size)]
                random_samples17 = [[random.randint(Control_X_low, Control_X_high-Control_X_size),] for _ in range(batch_size)]
                random_samples18 = [[random.randint(Control_Y_low, Control_Y_high-Control_Y_size) ] for _ in range(batch_size)]
                random_samples19 = [[random.uniform(Control_Var_low,Control_Var_high)] for _ in range(batch_size)]
                random_samples20 = [[random.uniform(Control_Var_low,Control_Var_high)] for _ in range(batch_size)]
                random_samples21 = [[random.randint(Control_X_low, Control_X_high-Control_X_size),] for _ in range(batch_size)]
                random_samples22 = [[random.randint(Control_Y_low, Control_Y_high-Control_Y_size) ] for _ in range(batch_size)]
                random_samples23 = [[random.uniform(Control_Var_low,Control_Var_high)] for _ in range(batch_size)]
                random_samples24 = [[random.uniform(Control_Var_low,Control_Var_high)] for _ in range(batch_size)]
                # リストを結合してサンプルを作成
                combined_samples = [sample1 + sample2+ sample3 + sample4 + sample5 + sample6+ sample7 + sample8 + sample9 + sample10+ sample11 + sample12 + sample13 + sample14 + sample15 + sample16 + sample17 + sample18 + sample19 + sample20 + sample21 + sample22 + sample23 + sample24 
                for sample1 , sample2, sample3 , sample4 , sample5 , sample6, sample7 , sample8 , sample9 , sample10, sample11 , sample12, sample13 , sample14 , sample15 , sample16 , sample17 , sample18 , sample19 , sample20 , sample21 , sample22 , sample23 , sample24  
                in zip(random_samples1 , random_samples2, random_samples3 , random_samples4 , random_samples5 , random_samples6, random_samples7 , random_samples8 , random_samples9 , random_samples10, random_samples11 , random_samples12, random_samples13 , random_samples14, random_samples15 , random_samples16 , random_samples17 , random_samples18, random_samples19 , random_samples20 , random_samples21 , random_samples22, random_samples23 , random_samples24)]
            ...
            TEST_prec_sum, CTRL_prec_sum, TEST_prec_matrix, CTRL_prec_matrix , TEST_CTRL_prec_matrix=sim(trial_i, optimal_inputs, f)#  change
