...
base_dir = f"../test_result_1run/{Control_purpose}_{optimal_inputs}_{current_time}"
...
def sim(LB_Control_X,LB_Control_Y,Control_MOMX, Control_MOMY, f):
    """
    ...
    for pe in range(nofpe):
        ...
        with netCDF4.Dataset(init) as src, netCDF4.Dataset(output, "w") as dst:
            ...
            for name, variable in src.variables.items():
                ...
                if name == "MOMX":
                    dst[name][:] = control_var_operation(data, pe, LB_Control_X, LB_Control_Y,
                                                        Control_Y_size, Control_X_size, 0, Control_MOMX)
                elif name == "MOMY":
                    dst[name][:] = control_var_operation(data, pe, LB_Control_X, LB_Control_Y,
                                                        Control_Y_size, Control_X_size, 0, Control_MOMY)
                else:
                    dst[name][:] = data
...
def main():
    ...
    with open(config_file_path,  "w") as f:
        ...
        TEST_prec_sum, CTRL_prec_sum, TEST_prec_matrix, CTRL_prec_matrix , TEST_CTRL_prec_matrix=sim(optimal_inputs[0],optimal_inputs[1],optimal_inputs[2], optimal_inputs[3],f)#  change
