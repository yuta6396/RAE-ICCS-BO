...
sum_no=np.zeros((24,90,90)) #制御前の累積降水量
# sum_prec=np.zeros((6,90,90))

cnt=0
# opt_num=100

for pe in range(nofpe): # 各プロセスごとに初期状態を設定
    init = init_file.replace('######', str(pe).zfill(6))
    org = org_file.replace('######', str(pe).zfill(6))
    subprocess.run(["cp", org, init]) #初期化

for i in range(24):
    subprocess.run(["mpirun", "-n", "4", "./scale-rm", "run.d02.conf"])
    for pe in range(nofpe):
        no_control=f"no-control_t={i}.pe######.nc"
...
for i in range(24):
    ...
    plt.savefig(f'../no-control_t={i}.png',dpi=300)
    plt.close()
    ...
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.savefig(f"../PREC_visualize_no-control_t={i}.png")
    plt.show()
