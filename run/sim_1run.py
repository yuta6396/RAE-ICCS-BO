...
Control_purpose = "Min_Max6hPREC" #Min_Max6hPREC, Min_Sum6hPREC
base_dir = f"../test_result_1run/Fig_{Control_purpose}_{optimal_inputs}_{current_time}"
...
def plot_PREC(TEST_prec_matrix, TEST_CTRL_prec_matrix, Objective_ratio):
    ...
    # plt_extent_sub = [137, 145, 34, 42]     # 詳細表示用の範囲
    ...
    # # ----- 四隅の座標を取得 -----
    # corners = [
    #     (slon[i_start, j_start], slat[i_start, j_start]),  # 左下
    #     (slon[i_start, j_end],   slat[i_start, j_end]),    # 右下
    #     (slon[i_end,   j_end],   slat[i_end,   j_end]),    # 右上
    #     (slon[i_end,   j_start], slat[i_end,   j_start])   # 左上
    # ]
    ...
    #                 zorder=10)
    # ax1.add_patch(polygon1) # 降水削減対象領域（矩形）を囲む
    ...
    plt.savefig(os.path.join(out_dir, f"LonLat_{Objective_ratio}%.png"), dpi=dpi_fig)
    ...
    #                 transform=ccrs.PlateCarree(),
    #                 zorder=10)
    # ax2.add_patch(polygon2) # 降水削減対象領域（矩形）を囲む
    ...
    # plt.savefig(os.path.join(out_dir, f"Sub_LonLat_{Objective_ratio}%.pdf"), dpi=1200)
    # plt.savefig(os.path.join(out_dir, f"Sub_LonLat_{Objective_ratio}%.pdf"), dpi=1200, bbox_inches = 'tight')
    plt.savefig(os.path.join(out_dir, f"Sub_LonLat_{Objective_ratio}%.png"), dpi=dpi_fig)
    ...
    plt.savefig(os.path.join(out_dir, f"Loop_by_loop_{Objective_ratio}%.png"), dpi=600)
