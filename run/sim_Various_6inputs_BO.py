...
# #　介入強度（固定）
Control_MOMX = 20   #30~30でエラー確認済み(MOMY)
Control_MOMY = -20
Control_QR = 0.05
Control_RHOT = 1.05
...
base_dir = f"../test_result_Various_6inputs/t=0-{Objective_T_period}_{Control_X_size}-{Control_Y_size}-{Control_Z_size}grids_FET={opt_num}_trials={trial_base}-{trial_base+trial_num-1}_{current_time}"
    ...
    for pe in range(nofpe):
        ...
        with netCDF4.Dataset(sub_init) as src, netCDF4.Dataset(output, "w") as dst:
            ...
            for name, variable in src.variables.items():
                ...
                var = src[name][:]
                if name == "MOMX":
                    dst[name][:] = control_var_add_operation(var, pe, inputs[0], inputs[1],  0, Control_MOMX)
                elif name == "MOMY":
                    dst[name][:] = control_var_add_operation(var, pe, inputs[0], inputs[1],  0, Control_MOMY)
                elif name == "QR":
                    dst[name][:] = control_var_add_operation(var, pe, inputs[2], inputs[3],  0, Control_QR)
                elif name == "RHOT":
                    dst[name][:] = control_var_product_operation(var, pe, inputs[4], inputs[5], 0, Control_RHOT)
                else:
                    dst[name][:] = src[name][:]
...
def plot_PREC(trial_i, TEST_prec_matrix, TEST_CTRL_prec_matrix, Objective_ratio):
    ...
    plt.savefig(os.path.join(out_dir, f"seed={trial_i}_TEST_{Objective_ratio}%.pdf"), dpi=1200)
    ...
    plt.savefig(os.path.join(out_dir, f"seed={trial_i}_TEST-CTRL_{Objective_ratio}%.pdf"), dpi=1200, bbox_inches = 'tight')
    ...
    plt.savefig(os.path.join(out_dir, f"seed={trial_i}_1h_loop_{Objective_ratio}%.pdf"), dpi=1200)
...
def main():
    ...
    with open(config_file_path,  "w") as f:
        ...
        # 介入強度
        f.write("\n# 介入強度\n")
        f.write(f"{Control_MOMX=}\n")
        f.write(f"{Control_MOMY=}\n")
        f.write(f"{Control_QR=}\n")
        f.write(f"{Control_RHOT=}\n")  
        # BOの獲得関数
        f.write("\n# BOの獲得関数\n")
    ...
    # 入力次元と最小値・最大値の定義
    bounds = [ Integer(Control_X_low, Control_X_high-Control_X_size), Integer(Control_Y_low, Control_Y_high-Control_Y_size),
               Integer(Control_X_low, Control_X_high-Control_X_size), Integer(Control_Y_low, Control_Y_high-Control_Y_size),
               Integer(Control_X_low, Control_X_high-Control_X_size), Integer(Control_Y_low, Control_Y_high-Control_Y_size),] # Int(2, 4) なら2, 3, 4からランダム
    ...
    with open(log_file, 'w') as f, open(summary_file, 'w') as f_s, open(analysis_file, 'w') as f_a:
        ...
        for trial_i in range(trial_base, trial_base+trial_num):
            ...
            for i in range(2):  # 最初の10点はRS
                ...
                # リストを結合してサンプルを作成
                combined_samples = [sample1 + sample2+ sample3 + sample4 + sample5 + sample6
                for sample1 , sample2, sample3 , sample4 , sample5 , sample6
                in zip(random_samples1 , random_samples2, random_samples3 , random_samples4 , random_samples5 , random_samples6)]
            ...
            plot_PREC(trial_i, TEST_prec_matrix, TEST_CTRL_prec_matrix, TEST_prec_sum/CTRL_prec_sum*100)
