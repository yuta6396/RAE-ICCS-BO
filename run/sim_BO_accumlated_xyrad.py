...
Control_Z_high = 2#MAX =36 36層存在 2の時429mが中心
...
base_dir = f"../test_result/accumlated_xySize/MOMXY+={Control_Var_size}_t=0-{Objective_T_period}_{Control_X_size}*{Control_Y_size}*{Control_Z_size}grids_FET={opt_num}_trials={trial_base}-{trial_base+trial_num-1}_{current_time}"
...
def predict(inputs, f):
    ...
    for i in range(batch_size):
        ...
        init_val_intervation(i,inputs[i][0],inputs[i][1],inputs[i][2], f) # ここで初期値を書き換える
...
def init_val_intervation(num,LB_Control_X,LB_Control_Y, Control_theta, f):
    """
    与えられた制御変数の値（LB_Control_X~4）を用いて初期値を変更する。
    """
    
    global org_file
    f.write(f"control LB_Control_X={LB_Control_X},LB_Control_Y={LB_Control_Y},Control_theta={Control_theta}\n")
    print(f"control LB_Control_X={LB_Control_X},LB_Control_Y={LB_Control_Y},Control_theta={Control_theta}\n")
    input_MOMX = Control_Var_size*np.cos(np.deg2rad(Control_theta) )
    input_MOMY = Control_Var_size*np.sin(np.deg2rad(Control_theta) )
    print(f"{input_MOMX=}, {input_MOMY=}\n")

    for pe in range(nofpe):
        output_file = f"000{num}/out.pe######.nc"
        ...
        with netCDF4.Dataset(sub_init) as src, netCDF4.Dataset(output, "w") as dst:
            ...
            for name, variable in src.variables.items():
                ...
                if name == "MOMX":
                    var = src[name][:]
                    dst[name][:] = control_var_operation(var, pe, LB_Control_X, LB_Control_Y, Control_Y_size, Control_X_size, 0 , input_MOMX)
                if name == "MOMY":
                    var = src[name][:]
                    dst[name][:] = control_var_operation(var, pe, LB_Control_X, LB_Control_Y, Control_Y_size, Control_X_size,0 , input_MOMY)
                else:
                    dst[name][:] = src[name][:]
...
def sim(LB_Control_X,LB_Control_Y,Control_theta):
    """
...
def plot_PREC(trial_i, TEST_prec_matrix, TEST_CTRL_prec_matrix):
    ...
    vmin = 0  # 最小値
    vmax = 160  # 最大値
    TEST_prec_sum_matrix = TEST_prec_matrix[1]
    for t in range(2, len(var)):
    ...
    ## 制御後- 制御前の降水強度
    TEST_CTRL_prec_sum_matrix = TEST_CTRL_prec_matrix[1]
    for t in range(2, len(var)):
...
def main():
    ...
    # 入力次元と最小値・最大値の定義
    bounds = [ Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size) ,Real(0,360)] # Int(2, 4) なら2, 3, 4からランダム
    ...
    with open(log_file, 'w') as f, open(summary_file, 'w') as f_s, open(analysis_file, 'w') as f_a:
        ...
        for trial_i in range(trial_base, trial_base+trial_num):
            ...
            random_samples4 = [[random.uniform(0,360)] for _ in range(batch_size)]

            # リストを結合してサンプルを作成
            combined_samples = [sample1 + sample2  + sample4 for sample1, sample2, sample4 in zip(random_samples1, random_samples2, random_samples4)]
