...
TIME_INTERVAL = 900 #TIME_INTERVAL[sec]ごとに降水強度を出力できる
varname = 'PREC'

init_file = "../init/init_d01_20070714-180000.000.pe######.nc"  
org_file = f"./interval={900}/restart_interval={TIME_INTERVAL}_t=40.pe######.nc"
history_file = f"history_interval={TIME_INTERVAL}.pe######.nc"
...
# 降水強度を最小化したい時間
Objective_T_period = int(6*(3600/TIME_INTERVAL)) # この場合0~6までの7回加算
...
"""
 Opt 300:
 BO・PSO:[71, 57, 20.0, -20.0]
 RS:[69, 57, 14.585247649846814, -19.88348217012944]
 GA: [71, 58, 20.0, -16.18597642]
"""

#optimal_inputs=[71, 57, 20.0, -20.0]
#optimal_inputs=[71, 57, 20, -20, 71, 57, 20, -20, 71, 57, 20, -20, 71, 57, 20, -20, 71, 57, 20, -20, 71, 57, 20, -20]
## MPC 300 BO min
optimal_inputs= [72, 57, 13.1231279, -20, 70, 61, 20,  -19.97732658, 70, 57, 20, -20, 68, 57, 20, -20, 70, 54, 20, -20, 73, 59, 20, 20]
"""
PSO 降水量の減少量 -7.54%
x[73. 78. 70. 76. 57. 70.]
y [54.  1. 58. 54. 72. 66.]
MOMX [ 12.20530586  -0.58699509  20.          20.         -19.8607375  -7.48016343]
MOMY [-20.         -20.         -20.          20.          19.13259487 9.36713335]
GA 降水量の減少量 -7.65%
x [64. 75. 68. 65. 73. 69.]
y [37. 58. 50. 55. 57. 50.]
MOMX [ 8.61900895  7.00998723 20.         -6.72602915 20.         13.34153646]
MOMY [ 15.99818057 -13.85354576 -16.79861985 -19.81995397  20. -16.14867746]
RS 降水量の減少量 -7.90%
x [72. 77. 72. 66. 66. 72.]
y [57. 66. 41. 56. 56. 64.]
MOMX [ 10.34471873  -7.1694491   17.05548789  14.22176009  14.22176009 -18.35495074]
MOMY[-13.87182623  19.14344492 -19.18998744 -18.37699157 -18.37699157 16.43762153]
"""
Control_purpose = "Min_Sum6hPREC" #Min_Max6hPREC, Min_Sum6hPREC
base_dir = f"../test_result_1run/{Control_purpose}_MPC_BO300_min_{current_time}"
        ...
        for pe in range(nofpe):
            ...
            with netCDF4.Dataset(init) as src, netCDF4.Dataset(output, "w") as dst:
                ...
                for name, variable in src.variables.items():
                    ...
                    if loop%4 == 0:
                        if name == "MOMX":
                            dst[name][:] = control_var_operation(data, pe, optimal_inputs[loop], optimal_inputs[loop+1], 0, optimal_inputs[loop+2])
                        elif name == "MOMY":
                            dst[name][:] = control_var_operation(data, pe, optimal_inputs[loop], optimal_inputs[loop+1], 0, optimal_inputs[loop+3])
                        else:
                            dst[name][:] = data
                    else:
                            dst[name][:] = data
...
def state_update(loop, TEST_prec_matrix, CTRL_prec_matrix, TEST_CTRL_prec_matrix, TEST_prec_sum, CTRL_prec_sum):
    orgfile = f"./interval={TIME_INTERVAL}/no-control_interval={TIME_INTERVAL}_t={loop+41}.pe######.nc"
    gpyoptfile=f"{base_dir}/interval={TIME_INTERVAL}_t={loop}.pe######.nc" 
    ...
    for pe in range(nofpe):
        ...
        if (os.path.isfile(history_path)):
            subprocess.run(["rm", history])
    subprocess.run(["mpirun", "-n", str(nofpe), "./scale-rm","run_interval=900.conf"])
...
def plot_PREC(TEST_prec_matrix, TEST_CTRL_prec_matrix, Objective_ratio):
    ...
    norm_diff = mcolors.Normalize(vmin=-80, vmax=80)
    norm_1h = mcolors.Normalize(vmin=5, vmax=35)
    norm_diff_1h = mcolors.Normalize(vmin=-20, vmax=20)
    ...
    for loop in range(Objective_T_period):
        ...
        history_name      = f"{base_dir}/interval={TIME_INTERVAL}_t={loop}"
        CTRL_history_name = f"/home/yuta/scale-5.5.3/scale-rm/test/tutorial/real/experiment_init_BO/run/interval={TIME_INTERVAL}/no-control_interval={TIME_INTERVAL}_t={loop+41}"
        ...
        # 単位変換 (例：kg/m/s → mm/h)
        var      = [v * 3600 for v in var]
        ctrl_var = [v * 3600 for v in ctrl_var]
        ...
        if loop == 0:
            ...
            prec_sum      = var[1].copy()/int(3600/TIME_INTERVAL)
            ctrl_prec_sum = ctrl_var[1].copy()/int(3600/TIME_INTERVAL)
        else:
            prec_sum      += var[1]/int(3600/TIME_INTERVAL)
            ctrl_prec_sum += ctrl_var[1]/int(3600/TIME_INTERVAL)
    ...
    # # ----- 3. グリッド上での累積降水量（TEST） -----
    # TEST_prec_sum = TEST_prec_matrix[0].copy()
    # for t in range(1, Objective_T_period):
    #     TEST_prec_sum += TEST_prec_matrix[t]
    # plt.figure(figsize=(10, 8))
    # plt.grid(True)
    # plt.imshow(TEST_prec_sum.T, cmap='viridis', aspect='auto', origin='lower', vmin=0, vmax=160)
    # plt.colorbar(label="Accumulated PREC (mm)")
    # plt.title(f"Time=0-{Objective_T_period}h_X={Objective_X_low}-{Objective_X_high-1}_Y={Objective_Y_low}-{Objective_Y_high-1}")
    # plt.xlabel("X")
    # plt.ylabel("Y")
    # plt.savefig(os.path.join(out_dir, f"Grid_{Objective_ratio}%.pdf"), dpi=1200)

    # # ----- 4. グリッド上での累積降水量（CTRL） -----
    # TEST_CTRL_prec_sum = TEST_CTRL_prec_matrix[0].copy()
    # for t in range(1, Objective_T_period):
    #     TEST_CTRL_prec_sum += TEST_CTRL_prec_matrix[t]
    # plt.figure(figsize=(10, 8))
    # plt.grid(True)
    # plt.imshow(TEST_CTRL_prec_sum.T, cmap='viridis', aspect='auto', origin='lower')
    # plt.colorbar(label="Accumulated PREC (mm)")
    # plt.title(f"Time=0-{Objective_T_period}h_X={Objective_X_low}-{Objective_X_high-1}_Y={Objective_Y_low}-{Objective_Y_high-1}")
    # plt.xlabel("X")
    # plt.ylabel("Y")
    # plt.savefig(os.path.join(out_dir, f"Grid_diff_C-noC_{Objective_ratio}%.pdf"), dpi=1200)
    ...
    for i in range(Objective_T_period):
        ...
        im = ax_sim.pcolormesh(slon, slat, prec_list[i], cmap=cmap, norm=norm_1h,
                               shading='auto', transform=ccrs.PlateCarree())
        hour = i//int(3600/TIME_INTERVAL)
        minite = i%int(3600/TIME_INTERVAL)                           
        if minite < int(3600/TIME_INTERVAL) - 1:
            ax_sim.set_title(f"TEST {hour}:{minite*15:02d}-{hour}:{(minite+1)*15}", fontsize=fs3)
        else:
            ax_sim.set_title(f"TEST {hour}:{minite*15}-{hour+1}:{0:02d}", fontsize=fs3)
    for i in range(Objective_T_period):
        ax_ctrl = axs[1, i]
        ...
        minite = i%int(3600/TIME_INTERVAL)                           
        if minite < int(3600/TIME_INTERVAL) - 1:
            ax_ctrl.set_title(f"CTRL {hour}:{minite*15:02d}-{hour}:{(minite+1)*15}", fontsize=fs3)
        else:
            ax_ctrl.set_title(f"CTRL {hour}:{minite*15}-{hour+1}:{0:02d}", fontsize=fs3)
    ...
    for i in range(Objective_T_period):
        ...
        minite = i%int(3600/TIME_INTERVAL)                           
        if minite < int(3600/TIME_INTERVAL) - 1:
            ax_ctrl.set_title(f"TEST - CTRL {hour}:{minite*15:02d}-{hour}:{(minite+1)*15}", fontsize=fs3)
        else:
            ax_ctrl.set_title(f"TEST - CTRL {hour}:{minite*15}-{hour+1}:{0:02d}", fontsize=fs3)   
    ...
    plt.savefig(os.path.join(out_dir, f"Loop_by_loop_{Objective_ratio}%.png"), dpi=300)
...
def main():
    ...
    with open(config_file_path,  "w") as f:
        ...
        TEST_prec_sum, CTRL_prec_sum, TEST_prec_matrix, CTRL_prec_matrix , TEST_CTRL_prec_matrix=sim(optimal_inputs,f)#  change
