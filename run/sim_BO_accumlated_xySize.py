...
base_dir = f"../test_result/accumlated_xySize/{Control_Var_name}_t=0-{Objective_T_period}_{Control_X_size}*{Control_Y_size}*{Control_Z_size}grids_FET={opt_num}_trials={trial_base}-{trial_base+trial_num-1}_{current_time}"
...
def init_val_intervation(num,LB_Control_X,LB_Control_Y, Control_Var, f):
    """
    与えられた制御変数の値（LB_Control_X~4）を用いて初期値を変更する。
    """
    
    global org_file
    f.write(f"control LB_Control_X={LB_Control_X},LB_Control_Y={LB_Control_Y},Control_Var={Control_Var}\n")
    print(f"control LB_Control_X={LB_Control_X},LB_Control_Y={LB_Control_Y},Control_Var={Control_Var}\n")
    for pe in range(nofpe):
        ...
        with netCDF4.Dataset(sub_init) as src, netCDF4.Dataset(output, "w") as dst:
            ...
            for name, variable in src.variables.items():
                ...
                if name == Control_Var_name:
                    var = src[name][:]
                    dst[name][:] = control_var_operation(var, pe, LB_Control_X, LB_Control_Y, Control_Y_size, Control_X_size, 0, Control_Var)
                else:
                    dst[name][:] = src[name][:]
...
def sim(LB_Control_X,LB_Control_Y,Control_Var):
    """
...
def main():
    ...
    # 入力次元と最小値・最大値の定義
    bounds = [ Integer(Control_X_low, Control_X_high-Control_X_size),Integer(Control_Y_low, Control_Y_high-Control_Y_size) ,Real(Control_Var_low,Control_Var_high)] # Int(2, 4) なら2, 3, 4からランダム
